...
def main(args):
    ...
    if args.write:
        ...
        out_file = args.write
        wrt = cv2.VideoWriter(
            out_file, cv2.VideoWriter_fourcc(*"mp4v"), fps, (1848, 720)
        )
    ...
    bev = BirdsEyeView(
        ...
        section_height=2 * 13.8826,
    )  # two dash distances in meters
    ...
    while True:
        ...
        if not ret:
            break
        log.info(
            "Processing frame {} ...".format(int(cap.get(cv2.CAP_PROP_POS_FRAMES)))
        )
        ...
        edges = (
            edg_fun(lab, gray, edg_primary, edg_secondary, edg_lcm, edg_threshold)
            * roi_mask_hard
        )
        ...
        img = cv2.addWeighted(img, 0.5, img_alpha, 0.5, 0)
        img_alpha = img.copy()
        ...
        if (left is not None) and (right is not None):
            ...
            img = cv2.addWeighted(
                img, (1 - measurement_alpha), img_alpha, measurement_alpha, 0
            )
        else:
            center_prev = None
        ...
        if left is not None:
            ...
            color = np.array(LaneColor.Valid.value) * alpha + np.array(
                LaneColor.Warning.value
            ) * (1 - alpha)
        ...
        text = (
            "Deviation from lane center: {0:.0f}% ({1:0.2}m)".format(
                deviation_from_center * 100, deviation_from_center_m
            )
            if deviation_from_center is not None
            else "Position: unknown"
        )
        cv2.putText(
            img, text, (4, 24), cv2.FONT_HERSHEY_DUPLEX, 0.75, (1, 1, 1), 1, cv2.LINE_AA
        )
        ...
        cv2.putText(
            img, text, (4, 48), cv2.FONT_HERSHEY_DUPLEX, 0.75, (1, 1, 1), 1, cv2.LINE_AA
        )
        ...
        #     motor.phai(speed=deviation_from_center)
        # elif (deviation_from_center < 0):
        #     motor.trai(speed=abs(deviation_from_center))
...
def parse_args():
    ...
    v.add_argument(
        "-w", "--write", dest="write", default=None, help="Writes an output video file"
    )
    v.add_argument(
        "-s",
        "--seek",
