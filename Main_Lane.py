...
cap = cv.VideoCapture(0) # 0 là số thứ tự của camera trong hệ thống của bạn
...
while True:
    try:
        ...
        # Calculate and display lane center
        def calculate_lane_center(lane_lines, frame_width):
            if len(lane_lines) < 2:
                return None

            left_line = lane_lines[0][0]  # Get the left lane line
            right_line = lane_lines[1][0]  # Get the right lane line

            # Calculate the midpoint between the two lanes
            x_center = (left_line[0] + right_line[0]) // 2
            y_center = (left_line[1] + right_line[1]) // 2
        ...
        lane_center_offset = calculate_lane_center(lane_lines, frame.shape[1])
    ...
    # Run Motor
        ...
        # if (lane_center_offset > 0):
        #     motor.phai(speed=lane_center_offset/100)
        # elif (lane_center_offset < 0):
        #     motor.trai(speed=abs(lane_center_offset)/100)
